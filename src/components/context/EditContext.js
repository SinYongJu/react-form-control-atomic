...
import { webpostDeleteOptionByid,webpostPostCreate , webpostGetOptionByid , webpostPostModify} from "../constants/edit_api";
...
export const EditProvider = ({ children, ...rest }) => {
  ...
  useEffect(() => {
    ...
    const getTarget = (id)=>{
      ...
      fetchCommon({...reqOption}).then(data => editSetMode(ctx => {
    ...
    if(getMode() === EDIT_PAGE_MODE.MODIFY || getMode() === EDIT_PAGE_MODE.DELETE){
      getTarget(id)
    }
  },[mode,id]);
