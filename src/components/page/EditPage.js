...
const ERROR_TEXT =
  "필수 항목 입니다. 특수문자 없이, 4글자 이상으로 작성해주세요.";
...
function EditPage() {
  ...
  useEffect(() => {
    if (editCtx.state === EDIT_STATE.INIT) {
      if (
        editCtx.inputTitle.value.length > EDIT_VALID_TEXT_LENGTH &&
        editCtx.inputDesc.value.length > EDIT_VALID_TEXT_LENGTH
      ) {
    ...
    editCtx.state,
    editCtx.isValid,
    editCtx.inputTitle.value,
    editCtx.inputDesc.value
  ]);
  ...
  useEffect(() => {
    editPageSetIsValid(
      validateInputValue(editCtx.inputTitle.value),
      validateInputValue(editCtx.inputDesc.value)
    );
